...
export default function KeyboardShortcutsModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-slate-900 dark:bg-slate-900 border border-slate-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
          {SHORTCUT_GROUPS.map((group, gIdx) => (
            <div key={gIdx} className="space-y-3">
              ...
              <div className="bg-slate-850/70 border border-slate-750/70 rounded-xl divide-y divide-slate-800/80 overflow-hidden">
                {group.items.map((item, idx) => {
                  const Icon = item.icon;
                  return (
                    <div
                      key={idx}
                      className="px-4 py-3 flex items-center justify-between gap-4 hover:bg-slate-800/40 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1.5">
                        {item.keys.map((k, kIdx) =>
                          k === 'or' ? (
                            <span key={kIdx} className="text-[10px] text-slate-500 font-medium">
                              or
                            </span>
                          ) : (
                            <kbd
                              key={kIdx}
                              className="px-2.5 py-1 rounded-md bg-slate-800 border border-slate-700 shadow-sm text-blue-300 font-mono text-[11px] font-bold"
                            >
                              {k}
                            </kbd>
                          )
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50 flex items-center justify-between text-xs text-slate-400">
          <span className="flex items-center gap-1.5">
            Press <kbd className="px-1.5 py-0.5 rounded bg-slate-800 text-[10px] text-slate-300 font-mono">Esc</kbd> anytime to dismiss
          </span>
          <button
            onClick={() => dispatch(setShortcutsModalOpen(false))}
