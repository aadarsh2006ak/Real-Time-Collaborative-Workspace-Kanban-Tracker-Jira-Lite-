...
export default function ProjectSettingsModal({ project, isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-colors">
        {/* Header */}
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900/80">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Project Settings</h3>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {activeTab === 'columns' && (
            <div className="space-y-6">
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300">Active Workflow Columns</h4>
                <div className="space-y-2">
                  {project.columns?.map((col) => (
                    <div
                      key={col._id}
                      className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-850/60 border border-slate-200 dark:border-slate-750 text-xs"
                    >
                      <div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400">
                          ...
                          <input
                            ...
                            className="w-14 px-2 py-1 rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-750 text-center text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
                          />
                        </div>
              ...
              {/* Add Column Form */}
              <form onSubmit={handleAddColumn} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-850/30 border border-slate-200 dark:border-slate-800 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="sm:col-span-2">
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-750 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
                    />
                  </div>
                  <div>
                    <input
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="px-4 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-md shadow-blue-500/20 disabled:opacity-50 transition-all"
                  >
                    Add Column
                  </button>
                </div>
          ...
          {activeTab === 'team' && (
            <div className="space-y-6">
              {/* Member List */}
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300">Project Members</h4>
                <div className="space-y-2">
                  {/* Owner Row */}
                  <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-850/60 border border-slate-200 dark:border-slate-750 text-xs">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-blue-600/20 border border-blue-500/30 text-blue-600 dark:text-blue-300 font-bold text-[10px] flex items-center justify-center">
                        {project.owner?.name ? project.owner.name[0].toUpperCase() : 'O'}
                      </div>
                      <div>
                        <p className="font-semibold text-slate-800 dark:text-slate-200">
                          {project.owner?.name} <span className="text-[10px] text-blue-600 dark:text-blue-400 font-mono">(Owner)</span>
                        </p>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400">{project.owner?.email}</p>
                      </div>
                    </div>
                  ...
                  {project.members?.map((m) => (
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-full bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-bold text-[10px] flex items-center justify-center">
                          {m.user?.name ? m.user.name[0].toUpperCase() : 'M'}
                        </div>
                        <div>
                          <p className="font-semibold text-slate-800 dark:text-slate-200">{m.user?.name || 'Member'}</p>
                          <p className="text-[10px] text-slate-500 dark:text-slate-400">{m.user?.email}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <select
                          ...
                          className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-750 text-xs text-slate-800 dark:text-slate-300 focus:outline-none focus:border-blue-500"
                        >
              ...
              {/* Invite Form */}
              <form onSubmit={handleInviteMember} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-850/30 border border-slate-200 dark:border-slate-800 space-y-3">
                <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="sm:col-span-2 relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-750 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
                    />
                  </div>
                  <div>
                    <select
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-750 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
                    >
                      <option value="viewer">Viewer</option>
                      <option value="member">Member</option>
                      <option value="admin">Admin</option>
            ...
            <form onSubmit={handleUpdateGeneral} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  disabled
                  value={project.key}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 font-mono text-xs cursor-not-allowed"
                />
                <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Project key is immutable.</p>
              </div>
              ...
              <div className="pt-3 flex justify-end">
                <button
                  type="submit"
                  disabled={isSaving}
                  className="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-lg shadow-blue-500/25 flex items-center gap-1.5 disabled:opacity-50 transition-all"
                >
                  {isSaving ? (
