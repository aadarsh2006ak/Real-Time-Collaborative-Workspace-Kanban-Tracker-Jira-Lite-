...
export default function CreateProjectModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden p-6 transition-colors">
        ...
        {errors.root && (
          <div className="mt-4 p-3 rounded-xl bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-500/30 text-rose-700 dark:text-rose-300 text-xs">
            {errors.root.message}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-750 text-slate-900 dark:text-white text-xs placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            />
            {errors.name && (
              <p className="mt-1 text-[11px] text-rose-500 dark:text-rose-400">{errors.name.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              {...register('key')}
              placeholder="MOB"
              className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-750 text-slate-900 dark:text-white text-xs uppercase font-mono tracking-wider placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            />
            {errors.key && (
              <p className="mt-1 text-[11px] text-rose-500 dark:text-rose-400">{errors.key.message}</p>
            )}
          </div>
